...
TEAM_COLORS = {
    ...
    "KIA 타이거즈": {"main": "#EA0029", "sub": "#FF6B6B"},
    "삼성 라이온즈": {"main": "#0066FF", "sub": "#4DA6FF"},
    "KT 위즈": {"main": "#FF003B", "sub": "#FF6680"},
    "두산 베어스": {"main": "#1A49C8", "sub": "#5C85FF"},
    "SSG 랜더스": {"main": "#CE0E2D", "sub": "#FF4D6D"},
    "롯데 자이언츠": {"main": "#003399", "sub": "#3366FF"},
    "한화 이글스": {"main": "#FF6600", "sub": "#FFAA00"},
    "NC 다이노스": {"main": "#0072CE", "sub": "#3399FF"},
...
st.markdown(f"""
    ...
    .fantasy-title {{
        ...
        text-align: center;
        letter-spacing: 2px;
        background: linear-gradient(180deg, #ffffff 0%, var(--team-glow) 50%, var(--team-sub-glow) 100%);
        -webkit-background-clip: text;
        -webkit-text-fill-color: transparent;
        filter: drop-shadow(3px 4px 0px #000000) drop-shadow(0px 0px 18px var(--team-glow));
    ...
    .fantasy-subtitle {{
        ...
        text-shadow: 0 0 12px var(--team-glow), 0 2px 4px rgba(0,0,0,0.9) !important;
    }}
    ...
    .fever-active-card {{
        ...
        box-shadow: 0 0 55px #ff6600, inset 0 0 35px #ff0033, 0 0 20px #ffff00 !important;
        animation: flameBorder 0.6s infinite alternate;
    }}

    @keyframes flameBorder {{
        0% {{ box-shadow: 0 0 40px #ff5500, inset 0 0 25px #ff0033; }}
        100% {{ box-shadow: 0 0 70px #ffcc00, inset 0 0 45px #ff3300; filter: brightness(1.15); }}
    }}
    ...
    .fever-bar-fill {{
        height: 100%;
        background: linear-gradient(90deg, #ff3300, #ff9900, #ffff00);
    ...
    .normal-bar-fill {{
        height: 100%;
        background: linear-gradient(90deg, var(--team-glow), var(--team-sub-glow));
        transition: width 0.1s linear;
    }}
    ...
    .fever-text {{
        ...
        text-shadow: 0 0 12px #ff3300, 0 2px 4px rgba(0,0,0,0.9) !important;
        margin-bottom: 8px;
    ...
    .energy-level-name {{
        ...
        font-weight: 900;
        color: #ffffff !important;
        text-shadow: 0 0 12px var(--team-glow), 0 0 25px var(--team-sub-glow), 0 2px 4px rgba(0,0,0,0.9) !important;
    }}

    /* 기타 UI 컴포넌트 텍스트 선명도 */
    .stSelectbox label, [data-testid="stMetricLabel"] {{
        color: #ffffff !important;
...
if "fever_start_time" not in st.session_state:
    st.session_state.fever_start_time = 0.0
if "fever_progress_clicks" not in st.session_state:
    st.session_state.fever_progress_clicks = 0

# 피버 타임 만료 체크 (9초 경과 시 종료)
if st.session_state.fever_active:
    if current_time - st.session_state.fever_start_time > 9.0:
        st.session_state.fever_active = False

# 기운 감쇠 로직 (2.5초 이상 방치 시)
time_passed = current_time - st.session_state.last_click_time
if time_passed > 2.5 and st.session_state.click_count > 0 and not st.session_state.fever_active:
    decay_amount = int((time_passed - 2.5) * 5)
...
if st.session_state.predict_result is None:
    with mega_card_placeholder.container():
        # 피버 타임 여부에 따른 상단 바 및 텍스트 렌더링
        if st.session_state.fever_active:
            rem_time = max(0.0, 9.0 - (time.time() - st.session_state.fever_start_time))
            pct = (rem_time / 9.0) * 100
            bar_html = f"""
                <div class="fever-bar-container"><div class="fever-bar-fill" style="width: {pct}%;"></div></div>
                <div class="fever-text">🔥 FEVER TIME! (남은 시간: {rem_time:.1f}초 | 3배 증폭 중!)</div>
            """
            card_extra_class = "standard-energy-card fever-active-card"
        else:
            prog = st.session_state.fever_progress_clicks
            pct = (prog / 60.0) * 100
            bar_html = f"""
                <div class="fever-bar-container"><div class="normal-bar-fill" style="width: {pct}%;"></div></div>
                <div class="energy-label-tag">⚡ 피버 게이지 ({prog} / 60 클릭)</div>
            """
            card_extra_class = "standard-energy-card"

        st.markdown(f"""
            <div class="{card_extra_class}">
                {bar_html}
                <div class="energy-level-name">{current_level_info["title"]}</div>
                <div style="font-size: 0.95rem; color: #e2e8f0; margin-top: 8px;">총 기운 스택: {st.session_state.click_count} / 500</div>
            </div>
        """, unsafe_allow_html=True)
...
with btn_col1:
    if st.button("⚡ 클릭하여 우주의 기운 모으기!", use_container_width=True):
        ...
        if st.session_state.fever_active:
            ...
            increment = 1
            st.session_state.fever_progress_clicks += 1
            # 60번 클릭 달성 시 피버 타임 진입 (피버 중에는 카운트 안 함)
            if st.session_state.fever_progress_clicks >= 60:
                st.session_state.fever_active = True
                st.session_state.fever_start_time = time.time()
                st.session_state.fever_progress_clicks = 0

        st.session_state.click_count = min(500, st.session_state.click_count + increment)
...
with st.expander("ℹ️ COSMIC PREDICT 알고리즘 및 기술 사양"):
    ...
    * **적용 핵심 기능:** 
      * 60클릭 달성 시 활성화되는 **9초간의 3배 증폭 피버 타임(Fever Time)** 및 불꽃 테두리 애니메이션
    ...
    * **작동 원리:** 평소에는 +1씩 오르던 기운이 피버 타임 동안 +3씩 증폭되며, 피버 타임 중에는 추가 스택 카운트가 유예됩니다.
    """)
